...
public class ResetAttributeMatcher extends BaseMatcher<ResetAttributeMatch> {
  ...
  protected ResetAttributeMatch tupleToMatch(final Tuple t) {
    try {
      return ResetAttributeMatch.newMatch((DseMergeDiffModel.ResetAttribute) t.get(POSITION_RESETATTROP), (wtspecid.IdentifiableWTElement) t.get(POSITION_TARGET));
  ...
  protected ResetAttributeMatch arrayToMatch(final Object[] match) {
    try {
      return ResetAttributeMatch.newMatch((DseMergeDiffModel.ResetAttribute) match[POSITION_RESETATTROP], (wtspecid.IdentifiableWTElement) match[POSITION_TARGET]);
    } catch(ClassCastException e) {
  ...
  protected ResetAttributeMatch arrayToMatchMutable(final Object[] match) {
    try {
      return ResetAttributeMatch.newMutableMatch((DseMergeDiffModel.ResetAttribute) match[POSITION_RESETATTROP], (wtspecid.IdentifiableWTElement) match[POSITION_TARGET]);
    } catch(ClassCastException e) {
      LOGGER.error("Element(s) in array not properly typed!",e);
      return null;
    }
