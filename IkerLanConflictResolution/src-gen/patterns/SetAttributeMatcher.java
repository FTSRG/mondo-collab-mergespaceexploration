...
public class SetAttributeMatcher extends BaseMatcher<SetAttributeMatch> {
  ...
  protected SetAttributeMatch tupleToMatch(final Tuple t) {
    try {
      return SetAttributeMatch.newMatch((DseMergeDiffModel.SetAttribute) t.get(POSITION_SETATTROP), (wtspecid.IdentifiableWTElement) t.get(POSITION_TARGET));
  ...
  protected SetAttributeMatch arrayToMatch(final Object[] match) {
    try {
      return SetAttributeMatch.newMatch((DseMergeDiffModel.SetAttribute) match[POSITION_SETATTROP], (wtspecid.IdentifiableWTElement) match[POSITION_TARGET]);
    } catch(ClassCastException e) {
  ...
  protected SetAttributeMatch arrayToMatchMutable(final Object[] match) {
    try {
      return SetAttributeMatch.newMutableMatch((DseMergeDiffModel.SetAttribute) match[POSITION_SETATTROP], (wtspecid.IdentifiableWTElement) match[POSITION_TARGET]);
    } catch(ClassCastException e) {
      LOGGER.error("Element(s) in array not properly typed!",e);
      return null;
    }
