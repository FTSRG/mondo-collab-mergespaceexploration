...
public class DeleteMatcher extends BaseMatcher<DeleteMatch> {
  ...
  protected DeleteMatch tupleToMatch(final Tuple t) {
    try {
      return DeleteMatch.newMatch((DseMergeDiffModel.Delete) t.get(POSITION_DELETEOP), (wtspecid.IdentifiableWTElement) t.get(POSITION_TARGET));
  ...
  protected DeleteMatch arrayToMatch(final Object[] match) {
    try {
      return DeleteMatch.newMatch((DseMergeDiffModel.Delete) match[POSITION_DELETEOP], (wtspecid.IdentifiableWTElement) match[POSITION_TARGET]);
    } catch(ClassCastException e) {
  ...
  protected DeleteMatch arrayToMatchMutable(final Object[] match) {
    try {
      return DeleteMatch.newMutableMatch((DseMergeDiffModel.Delete) match[POSITION_DELETEOP], (wtspecid.IdentifiableWTElement) match[POSITION_TARGET]);
    } catch(ClassCastException e) {
      LOGGER.error("Element(s) in array not properly typed!",e);
      return null;
    }
